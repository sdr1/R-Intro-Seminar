...
# esc gets you out of a lot of errors

If you can't type a command because of lines and lines of the **+** symbol, just hit esc.

For example, you may run into the following scenario:

```{r, eval=F}
data <- readr::read_csv("https://raw.githubusercontent.com/nppackages/binsreg/master/R/binsreg_sim.csv)
+ 
+ 
```

To get out of it, hit esc.  
...
Open a new tab in R (File -> New File -> R Script)
...
Why will the following cause an error? To figure it out, recall the steps from <https://sdr1.github.io/R-Intro-Seminar/2-intro-to-coding-debugging.html#Debugging>
...
#' I.e. First, type the dataset that you want to analyze/do something to 
storms %>%
  # Now link the dataset to another command - here it's select 
  # select variables
...
You'll likely need to create functions and loops to analyze/parse data.

Here is the structure of a basic for loop.  We loop over the numbers 32 to 100, and, for each element of the loop apply the formula to convert Fahrenheit into Celsius.

```{r}

loop_over_this <- 32:100 # you can experiment with any numeric range

# making C a blank list here
C <- list()

for(i in loop_over_this){
  # What are you looping over
  print(i)
  # If we pretend that the i's are degrees Fahrenheit and we want to convert them into Celsius we can do the following
  C[i] <- (i - 32) * 5/9
  print(C[i])
}

tibble(Fahrenheit = loop_over_this,
       Celsius = unlist(C))
```

We can also do this as a function.

```{r}

F_to_C_Converter <- function(Fahrenheit){
  C = (Fahrenheit - 32) * 5/9
  return(C)
}

F_to_C_Converter(77)
F_to_C_Converter(1:100)
```
...
Dplyr makes summarizing across one variable moderatly easy.
```{r Summary stats}
mtcars %>% 
  summarise(
            n = n(),
            ...
            max = max(mpg),
            sd = sd(mpg)) %>%
  #' This stuff below just makes the data above display nicely for you, its completely unnecessary for most uses.
  kable(digits = 2) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))  %>%
  scroll_box(width = "900px", height = "200px")
```

Suppose, however, you wanted to summarize across multiple variables.  dplyr makes this a pain. The psych library, however, makes it a little easier.  Just use the following code.  The key here is that data.frame() allows us to keep the variable names.  Without it, we wouldn't have names for the variables.
...
Skim is a great package for summarizing variables - unless you need a variable they don't have.  Then good luck. See <https://cran.r-project.org/web/packages/skimr/vignettes/extending_skimr.html>
...
Take a data set, figure out how the variables are stored, and give me the median, mean, standard deviation, 75th and 95th percentile of hlth. hlth is the processed version of health. To see the difference between the two, just swap them out in the exercise below and you'll notice a difference.
...
NHIS %>%  
  ...
  rownames_to_column(var = "name") %>%
  tibble() %>%
  select(name, n, median, mean, sd, Q0.75, Q0.95) %>%
  mutate(across(where(is.numeric), round, 1)) 
...
OLS is the ordinary (as in all elements are treated equally) least squares estimate of the parameter $\beta$ that minimizes $\epsilon'$$\epsilon$. To save you a lot of time, the result is that $\hat{\beta} = (X'X)^{-1}X'Y$. 
